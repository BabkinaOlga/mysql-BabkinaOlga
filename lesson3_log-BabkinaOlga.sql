...
CREATE TABLE media (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT "Идентификатор медиафайлов",    
  filename VARCHAR(100) NOT NULL COMMENT "Имя медиафайла",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания медиафайла",
  content BLOB COMMENT "Содержание медиафайла"
  );
...
CREATE TABLE coment (
  ...
  sender INT UNSIGNED NOT NULL COMMENT "Создатель поста",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания поста",
  content BLOB COMMENT "Содержание поста"
  );
...
CREATE TABLE likes (
  ...
  coment_id INT UNSIGNED NOT NULL COMMENT "Идентификатор поста",
  media_id INT UNSIGNED NOT NULL COMMENT "Идентификатор медиафайлов",
  user_id INT UNSIGNED NOT NULL UNIQUE COMMENT "Идентификатор пользователя" 
  );
  
 -- Создаем внешние ключи для связи таблицы "likes" с "users", "media", "coment".
ALTER TABLE likes ADD CONSTRAINT fk_user_id FOREIGN KEY (user_id) REFERENCES users(id);
ALTER TABLE likes ADD CONSTRAINT fk_media_id FOREIGN KEY (media_id) REFERENCES media(id); 
ALTER TABLE likes ADD CONSTRAINT fk_coment_id FOREIGN KEY (coment_id) REFERENCES coment(id)
